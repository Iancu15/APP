...
int main (int argc, char **argv) {
    ...
    int suma = sum_array(process_arr, chunks_per_process);
    ...
    * fiecarui proces
    */
    MPI_Gather(&suma, 1, MPI_INT, max_arr, 1, MPI_INT, ROOT, MPI_COMM_WORLD);

    if (rank == ROOT) {
        // maximul aflat din array-ul de maximuri locale
        int max_maximum = sum_array(max_arr, proc);
        printf("Maximum found with scatter and gather: %d\n", max_maximum);

        // maxim aflat pe tot array-ul original
        int total_max = sum_array(arr, proc * chunks_per_process);
